...
### feature ranking function; takes in input the whole dataset and the list of sample indices for the training set
...
## load data
all_data <- read.table("~/Google Drive/chicco_survival/data/dataset_edited_without_time_NORM.csv",sep=",",header=TRUE)
...
for(i in 1:n_sets){
  ...
  for(nn in methods)  splits[[i]][["ranked_list"]][[nn]] <- ranker(data=all_data,train_indices=splits[[i]][["TR"]],method=nn)
}
...
Rf <- unlist(lapply(splits,FUN = function(x){x[["top_feats"]][["rf"]][["TS"]][["MCC"]]}))
Svm <- unlist(lapply(splits,FUN = function(x){x[["top_feats"]][["svm"]][["TS"]][["MCC"]]}))
Xgb <- unlist(lapply(splits,FUN = function(x){x[["top_feats"]][["xgb"]][["TS"]][["MCC"]]}))
